/*  A Swift implementation of the good old FizzBuzz simple programming test
    If you are not familiar with what the problem is, check it out at https://www.hackerrank.com/challenges/fizzbuzz

    This is (admittidly) an over-bloated solution for such a simple problem, however, I created it to practice writing production-level, testable, and maintainable code (ex. You can apply Unit Tests against the functions), and also to practice Functional Programming principles (ex. `fizzBuzzLine()` is a pure function, while `printFizzBuzz()` isn't.
*/
